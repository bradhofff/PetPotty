/*
  Supports the simplified Add/Edit Vet Visit forms.
  - Address and phone are no longer required by the stored procedures.
  - PreparationInstructions is consolidated into Notes without losing old data.
  - Notes becomes nvarchar(max) so both legacy text fields fit safely.

  The legacy procedure parameters remain optional for compatibility with an
  older application build during a rolling deployment.
*/
SET XACT_ABORT ON;
GO
...
BEGIN
    ...
    DECLARE @CombinedNotes nvarchar(max) = CASE
        ...
        ELSE CONCAT(LTRIM(RTRIM(@PreparationInstructions)),
                    CHAR(13), CHAR(10), CHAR(13), CHAR(10), LTRIM(RTRIM(@Notes)))
    END;
